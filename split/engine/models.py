...
class cliente(models.Model):
    nombres = models.CharField(max_length=64,unique=False)                                                           # nombres del cliente que acompaña al usuario
    apellidos = models.CharField(max_length=64,unique=False)                                                         # apellidos del cliente que acompaña al usuario
    correo = models.EmailField(max_length=254,unique=False)                                              # correo electronico al que se enviara el detalle de la cuenta y su consumo personal
...
class orden(models.Model):
    ...
    cuenta = models.ForeignKey(cuenta,on_delete=models.CASCADE,null=True)                     # cuenta en la que esta asociado el consumo
    dividido = models.BooleanField(default=False)                                  # si un producto es de consumo disparejo una persona puede consumir mas que los demas por lo que cada usuario paga lo correspondiente a su consumo
    cantidad_total = models.IntegerField(blank=True,null=True)
    consumos = models.ManyToManyField("consumo",related_name="orden_consumos",blank=True)
    subtotal = models.DecimalField(decimal_places=2,max_digits=16,default=0,blank=True,null=True)
    subtotal_iva = models.DecimalField(decimal_places=2,max_digits=16,default=0,blank=True,null=True)
    class Meta:
        verbose_name= "Orden"
        verbose_name_plural ="Ordenes"
    def __str__(self):
        return f"{self.pk}: {self.item} {self.cuenta} {self.dividido} {self.consumos}"
...
class consumo(models.Model):
    ...
    def __str__(self):
        return f"{self.pk}: {self.cliente} {self.cantidad}"
    def Item(self):
        return self.item
